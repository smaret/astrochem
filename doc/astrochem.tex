...
Several networks are distributed with Astrochem. These networks can be
found in the network directory of the source distribution.
...
Astrochem comes with a tool called \verb=chmconvert= to convert
network files to \verb=.chm= files. For the moment, this tool supports
only the OSU database format. The type of file is determined from the
file extension, which must be \verb=.osu= for Ohio State University
database files. Networks can be converted as follows:
