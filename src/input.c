      ...
      else if (sscanf (line, "%s = %s", parameter, value) == 2)
	{
	  ...
	  /* Output */
		   ...
		   copy them in the ouput_species array. */
		{
		  ...
		  species_idx = find_species (output_specie, network);
		  if (species_idx < 0)
		    {
		      fprintf (stderr,
...
read_source (const char *source_file, mdl_t * source_mdl, const int verbose)
{
  ...
  n_line = get_nb_active_line (source_file);
  ...
  /* Loop over the lines, and read the source model. */
  ...
  while (fgets (line, MAX_LINE, f) != NULL)
	...
	continue;		/* Skip comments */
      ...
      /* Allocate the source structure if not done already. */
